...
// __uint128_t (pair capacities 64 / 128 / 256 bits) and pick, per requested
// k/m/b, the smallest type that fits. dispatch_width_bytes() turns the chosen
// byte width back into a concrete template instantiation at the call site.
...
// Smallest precompiled kuint width (in bytes: 4, 8, or 16) whose pair holds
// `need_bits`. Throws if even __uint128_t's 256-bit pair is too small.
inline uint64_t select_width_bytes(uint64_t need_bits) {
    ...
    throw std::runtime_error(
        ...
        " bits but the widest backend (__uint128_t) holds 256. Reduce k, or raise m.");
}
