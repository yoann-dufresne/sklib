...
class SortedVirtualSkmerList {
    ...
    // .md, SubstrateHoleMonotonicityFailsAtScale) found that the per-column key becomes
    // monotone with holes included only on TRIVIALLY SMALL inputs; at genome scale holes
    // break monotonicity (a hole's high-order content at a column where it is absent is
    // already out of order, and no absent-bit fill can fix high-order bits), so a
    // hole-aware query is incorrect there. The fill is therefore currently a no-benefit
    ...
    void fill_absent_sentinel() {
        ...
        for (Skmer<kuint>& sk : m_skmer_list) {
            const kpair amask = m_manip.absent_slot_mask(sk.m_pref_size, sk.m_suff_size);
            sk.m_pair.m_value[0] = static_cast<kuint>(sk.m_pair.m_value[0] & ~amask.m_value[0]);
            sk.m_pair.m_value[1] = static_cast<kuint>(sk.m_pair.m_value[1] & ~amask.m_value[1]);
        }
    }
