...
#### Absent-slot sentinel (investigation — negative result)

A stored super-k-mer that covers few k-mers leaves its peripheral flank slots unused (build-time padding). An experiment cleared those absent slots to a sentinel (the entry's minimal completion), hoping to let a membership query binary-search *through* the entries that lack a k-mer at the searched position instead of linearly scanning over them. The fill is transparent to the current query (it rewrites only bits masked out when comparing valid entries, so the on-disk format and all query results are unchanged), **but it does not achieve its goal**: the per-column search key becomes monotone with those entries included only on trivially small inputs — at genome scale the construction order (which is *not* the `m_pair` order) leaves them out of place, so a hole-aware binary search is incorrect (~5% false negatives on a 2 Mb genome). See [`docs/sentinel_substrate.md`](docs/sentinel_substrate.md) for the full analysis, measurements, and the path forward (a per-column index to skip those entries, rather than a bit fill).
